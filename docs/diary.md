...
The label didn't last the day. John folded its job into the status line, which now always says how the list is sorted, then names the other two buttons: "1,853 songs, sorted by artist. Pick Title or Album to sort that way." The hint changes with the sort and goes away when nothing matches. Screen readers hear the first sentence as it changes, but not the hint on every keystroke, and the group of buttons is still named "Sort by" for them.
...
**Then a prune.** John noticed the song count showing three times: in the line under the title, on the All songs button, and in the status line. Claude mocked up a rule, each fact shown once, where it's most useful, and John kept one exception: the Sad and Scary buttons keep their counts. The count lives in the status line above the songs, the one line that follows the lists and the search. The date sits alone under the title. The page's own text, a link to a Spotify playlist, now comes after the songs. On a 375px phone the first song moved up 114px, and the first screen shows four songs instead of two. Later that day John put the total back on All songs, so every list button shows its size, renamed the List label "Lists", and moved the date to the foot of the songs. The heading now names the list picked, as the old site's post titles did: "Full Songlist", "Sad Songs Only", "Scary Songs Only", built from each theme's name, so a new list gets its heading for free.
